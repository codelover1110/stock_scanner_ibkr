$(document).ready(function () {
  ...
  const histogramChartV2 = new Chart(document.getElementById("histogramChartV2"), {
      type: "bar",
      data: {
          labels: data.map((row) => row.date),
          datasets: [
              {
                  label: "Rolling Sharpe Ratio - 30 Seconds",
...
const meanLinesPlugin = {
  ...
  afterDatasetDraw: (chart, args, options) => {
      const { ctx,chartArea:{top,bottom,right,left} } = chart;
      ctx.save();
      ctx.beginPath();;
